...
docker run -p 8080:8080 -e KEYCLOAK_USER=admin -e KEYCLOAK_PASSWORD=password quay.io/keycloak/keycloak:20.0.5
...
> 使用shadow打完整包"keycloak-login-spi-20.0.5-all.jar"

```shell
# keycloak-login-spi-20.0.5-all.jar 传到 `/opt/keycloak/providers` 目录 -> 然后重启pod
curl -s -k -L --output keycloak-login-spi-20.0.5.jar https://github.com/yaoguoh/keycloak-extensions/releases/download/20.0.5/keycloak-login-spi-20.0.5-all.jar
```
